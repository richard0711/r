...
class ContentItem extends CI_Model {
    
    protected $table = "content_items";
    protected $primary_key = "idcontent_item";
    ...
    public function get_content_items_by_filters($filters = array()){
        ...
        $this->db->join("images", "images.idimage=content_items.item_id");
        $this->db->where($this->table.".status", 1);
        $result['count'] = $this->db->count_all_results('', false);
        set_query_limit_and_offset($filters, $this->db);
        $res = $this->db->get();
        log_message('debug', 'get_content_items_by_filters $this->db->last_query()'.print_r($this->db->last_query(), true));
