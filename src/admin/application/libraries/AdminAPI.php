...
class AdminAPI {
    ...
    public function get($route = '', $params = array()) {
        ...
        $result = $Curl->simple_get(ADMIN_API_URL . $route, $params);
    ...
    public function post($route = '', $params = array()) {
        $Curl = get_curl($this->_getCurlOptions());
        $result = $Curl->simple_post(ADMIN_API_URL . $route, json_encode($params));
        return $this->processCurlRes($Curl, $result);
    }
    
    protected function processCurlRes($Curl = null, $result = null) {
        log_message('debug', 'AdminAPI->get curl result ' . print_r($result, true));
        log_message('debug', 'AdminAPI->get curl info'.print_r($Curl->info, true));
        ...
        if (!$result || $result === null) {
            log_message('error', 'get webshop filter curl '.$Curl->error_string . ': ' . $Curl->error_code);
