...
class User extends CI_Model {
    ...
    public function get_users_by_filters($filters = array()){
        ...
        if (isset($filters["email"]) && $filters["email"] != '') {
            $this->db->where($this->table.".email", $filters["email"]);
        }
        if (isset($filters["system"]) && $filters["system"] != '') {
            $this->db->select('user_system.iduser_system');
            $this->db->join("user_system", "user_system.iduser=users.iduser");
        ...
        $res = $this->db->get();
        log_message('debug', 'get_users_by_filters $this->db->last_query()'.print_r($this->db->last_query(), true));
