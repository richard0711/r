...
class Position extends CI_Model {
    ...
    protected $primary_key = "idposition";
    ...
    public function get_positions_by_filters($filters = array()){
        $result = array('count' => 0, 'data' => array());
        $this->db->from($this->table);
        $this->db->select($this->table.'.*');
        if (isset($filters["idposition"]) && $filters["idposition"] > 1) {
            $this->db->where($this->table.".idposition", $filters["idposition"]);
        }
        if (isset($filters["type"])) {
            $this->db->where($this->table.".type", $filters["type"]);
        }
        $this->db->where($this->table.".status", 1);
        $result['count'] = $this->db->count_all_results('', false);
        set_query_limit_and_offset($filters, $this->db);
        $res = $this->db->get();
        log_message('debug', 'get_positions_by_filters $this->db->last_query()'.print_r($this->db->last_query(), true));
